...
- **予約の変更には社長の承認が必要。** `content/queue/` の `status` を `ready` にする・`scheduled_for` を
  変える・予約を取り消す操作は、その会話の中で社長が**具体的な投稿 ID と日時を明示して承認した場合だけ**行う。
  別のセッションや過去の会話での承認は流用しない。例外は、投稿の失敗が続くなど事故を止めるために
  `ready` を `draft` に戻す場合だけで、そのときは理由を社長に報告する
- 予約を変えたら、変更後の予約一覧（投稿 ID・日時）を社長に見せる
...
4. ~~投稿系~~ 完了（カルーセル・リール。失敗した投稿は `status=error` で止まる）
